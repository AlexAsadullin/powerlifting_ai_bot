...
from models.models import Student, GroupStudent, PaymentRequest, Trainer, Group, Schedule
...
async def handle_trainer_sessions(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        if pending_request:
            await message.answer(f"Ваш запрос на {pending_request.sessions_requested} занятий находится на рассмотрении.")
        else:
